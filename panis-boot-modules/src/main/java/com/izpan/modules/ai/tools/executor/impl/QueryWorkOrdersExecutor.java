...
public class QueryWorkOrdersExecutor implements IAiToolExecutor {
    ...
    public AiToolResult execute(Map<String, Object> arguments) {
        ...
        try {
            Integer orderStatus = arguments.containsKey("orderStatus") 
                    ? ((Number) arguments.get("orderStatus")).intValue() : null;
            Integer priority = arguments.containsKey("priority") 
                    ? ((Number) arguments.get("priority")).intValue() : null;
            ...
            if (orderStatus != null) {
                queryWrapper.eq(WorkOrder::getOrderStatus, orderStatus);
            }
            if (priority != null) {
            ...
            return AiToolResult.failure(getToolName(), "查询工单列表失败: " + e.getMessage());
        }
    }
