...
public class SysUserServiceImpl extends ServiceImpl<SysUserMapper, SysUser> implements ISysUserService {
    ...
    public boolean updateCurrentUserInfo(SysUserBO sysUserBO) {
        boolean updateById = super.updateById(sysUserBO);
        // 自我更新个人资料，需要更新缓存资料
        SysUser sysUser = CglibUtil.convertObj(sysUserBO, SysUser::new);
        saveUserToSession(sysUser, true);
        return updateById;
    }
    ...
    @Override
    public IMonLogsLoginService getMonLogsLoginService() {
        return monLogsLoginService;
    }
    ...
    @Override
    public void saveUserToSession(SysUser user, boolean needCheck) {
        if (needCheck) {
            user = super.getById(user.getId());
        }
        // 用户转换
        LoginUser loginUser = CglibUtil.convertObj(user, LoginUser::new);
        // 获取用户角色
        List<SysRoleBO> sysRoleBOS = sysRoleService.queryRoleListWithUserId(user.getId());
        ...
        Set<Long> userOrgIds = sysUserOrgService.queryOrgUnitsIdsWithUserId(user.getId());
        loginUser.setOrgIds(userOrgIds);
        // Session 放入用户对象
        StpUtil.getSessionByLoginId(user.getId()).set("user", loginUser);
    }

    @Override
    public boolean saveLoginLog(MonLogsLogin loginLog) {
        return monLogsLoginService.save(loginLog);
    }
